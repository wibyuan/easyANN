...
namespace {
    ...
    void KNN_search_internal(const int16_t* padded_query, int *res){
        ...
        while(!C.empty()){
            auto c = C.top(); C.pop();
            if(Bk.size() >= K && c.first > (1.0f + gamma_) * Bk.top().first) break;
            ...
            for(int i = 0; i < c_sz; ++i){
                ...
                if(mass[e] != curV){
                    ...
                    if(Bk.size() < K || xx < (1.0f + gamma_) * Bk.top().first)
                        C.push(make_pair(xx, e));
                }
            }
        }
...
vector<int> Solution::get_degree_distribution(){
    ...
    degrees.reserve(N);
    for(int i = 0; i < N; ++i){
        degrees.push_back(get_neighbor_count(i));
    }
    return degrees;
}
...
void Solution::save_graph(const string& path){
    ...
    fout.write(reinterpret_cast<const char*>(&dim_padded), sizeof(int));
    fout.write(reinterpret_cast<const char*>(&enter_point), sizeof(int));
    fout.write(reinterpret_cast<const char*>(&g_min_val), sizeof(float));
    fout.write(reinterpret_cast<const char*>(&g_scale), sizeof(float));
    fout.write(reinterpret_cast<const char*>(&size_per_element_), sizeof(size_t));
    fout.write(reinterpret_cast<const char*>(&offset_links_), sizeof(size_t));
    fout.write(reinterpret_cast<const char*>(&offset_data_), sizeof(size_t));

    int map_size = reordered_to_original.size();
    fout.write(reinterpret_cast<const char*>(&map_size), sizeof(int));
    if(map_size > 0)
        fout.write(reinterpret_cast<const char*>(reordered_to_original.data()), map_size * sizeof(int));

    size_t total_size = (size_t)N * size_per_element_;
    fout.write(memory_pool_, total_size);
    fout.close();
}
...
bool Solution::load_graph(const string& path){
    ...
    fin.read(reinterpret_cast<char*>(&dim_padded), sizeof(int));
    fin.read(reinterpret_cast<char*>(&enter_point), sizeof(int));
    fin.read(reinterpret_cast<char*>(&g_min_val), sizeof(float));
    fin.read(reinterpret_cast<char*>(&g_scale), sizeof(float));
    fin.read(reinterpret_cast<char*>(&size_per_element_), sizeof(size_t));
    fin.read(reinterpret_cast<char*>(&offset_links_), sizeof(size_t));
    fin.read(reinterpret_cast<char*>(&offset_data_), sizeof(size_t));

    int map_size;
    fin.read(reinterpret_cast<char*>(&map_size), sizeof(int));
    reordered_to_original.resize(map_size);
    if(map_size > 0)
        fin.read(reinterpret_cast<char*>(reordered_to_original.data()), map_size * sizeof(int));

    if(memory_pool_) aligned_free_wrapper(memory_pool_);
    size_t total_size = (size_t)N * size_per_element_;
    memory_pool_ = (char*)aligned_alloc_wrapper(total_size, 64);
    if(!memory_pool_) { fin.close(); return false; }
    fin.read(memory_pool_, total_size);
