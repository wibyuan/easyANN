...
namespace {
    ...
    const int K=10, efConstruction=200, Mmax0=200;
    const float epsilon=0.22,gamma=0.21;
    ...
    void KNN_build(int d, const vector<float>& base){
        ...
        shortcut_reduction(N);
        // int cnt[Mmax0+1]={0};
        // for(int i=0;i<N;++i)
        //     ++cnt[get_neighbor_count(i)];
        // for(int i=0;i<=Mmax0;++i)
        //     cout<<i<<":"<<cnt[i]<<'\n';
        reorder_memory(N);
